...
-- Initialize unified SearchManager for items
if ClientSearchManager then
    ClientSearchManager.InitSearch("items", {
        pageSize = 50,
        debounce = {
            enabled = false  -- Manual search button, no auto-debounce
        }
    })

    -- Register callback for item search results
    ClientSearchManager.RegisterCallback("items", function(results, pagination)
        PlayerInventory.receiveItemSearchResults(nil, results, pagination)
    end)

    if GMConfig and GMConfig.config and GMConfig.config.debug then
        print("[ItemSearch] Initialized with unified SearchManager")
    end
end
    ...
    performSearch = function()
        ...
        if ClientSearchManager then
            ...
            local filters = {
                ...
                quality = dialog.selectedQuality,
                slotId = (dialog.isEquipment and dialog.slot and dialog.slot.slotId) or nil,
                minLevel = minLevel,
                maxLevel = maxLevel,
                minItemLevel = minILevel,
                maxItemLevel = maxILevel,
                stackableOnly = stackableOnly,
                sortBy = sortBy,
                sortOrder = sortOrder
            }

            ClientSearchManager.Search("items", searchText, filters, true)
        else
            -- Fallback to old direct AIO call
...
function PlayerInventory.receiveItemSearchResults(player, results, pagination)
    ...
    if pagination and pagination.totalCount and pagination.totalCount > 0 then
        dialog.resultsText:SetText(string.format("Search Results: %d-%d of %d",
    ...
    for i, item in ipairs(results) do
        ...
        resultFrame:SetScript("OnEnter", function(self)
            ...
            -- Show tooltip using cache-aware function
            GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
            GMUtils.SetItemTooltip(GameTooltip, item.entry)
            GameTooltip:Show()
        end)

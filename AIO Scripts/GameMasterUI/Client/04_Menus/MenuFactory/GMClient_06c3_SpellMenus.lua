...
function SpellMenus.createAuraSubmenu(spellId)
    return {
        ...
        {
            text = "Apply with Options...",
            func = function()
                -- Check if AuraOptions module is available
                if _G.GMMenus and _G.GMMenus.SpellSelection and _G.GMMenus.SpellSelection.AuraOptions then
                    -- Get spell name from spell ID (if available)
                    local spellName = GetSpellInfo(spellId) or "Spell " .. spellId
                    _G.GMMenus.SpellSelection.AuraOptions.showEntityAuraOptionsDialog(spellId, spellName)
                else
                    print("[ERROR] AuraOptions module not loaded. Please restart the server.")
                end
            end,
            notCheckable = true,
            tooltipTitle = "Advanced Options",
            tooltipText = "Configure duration, stack count, and target for this aura",
        },
        {

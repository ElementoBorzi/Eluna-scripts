...
function ItemMenus.createItemMenu(entity)
    ...
    return {
            ...
            if _G.ItemTemplateEditor then
                ...
                if StateMachine then
                    if not StateMachine.canOpenModal() then
                        print("[ItemTemplateEditor] Cannot open - system busy")
                        return
                    end
                    -- Use state machine for coordinated opening
                    if not StateMachine.openItemEditor(trimmedEntry) then
                        print("[ItemTemplateEditor] State machine transition failed - using fallback")
                        _G.ItemTemplateEditor.Open(trimmedEntry, false)
                    end
                else
                ...
                print("[ERROR] ItemTemplateEditor not loaded!")
            end
        end, {
            title = "Edit Template",
            text = "Open the Item Template Editor to modify this item's properties"
        }),
        {
        ...
        MenuItems.createTemplateItem("Duplicate as Template...", function()
            if _G.ItemTemplateEditor then
                -- Check state machine availability and use it for coordination
                local StateMachine = _G.GMStateMachine
                if StateMachine then
                    ...
                    -- Use state machine for coordinated opening
                    if not StateMachine.openItemEditor(trimmedEntry, true) then
                        print("[ItemTemplateEditor] State machine transition failed - using fallback")
                        _G.ItemTemplateEditor.Open(trimmedEntry, true)
                    end
                else
                    -- Fallback if state machine not available
                    _G.ItemTemplateEditor.Open(trimmedEntry, true)
            ...
            text = "Create a new item using this item as a template"
        }),

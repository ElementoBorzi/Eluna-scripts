...
function PlayerInventoryDataHandlers.queryAndSendInventory(player, targetName)
    ...
    -- DISABLED: Send item packets for all inventory items before sending inventory data
    ...
    if _G.GameMasterUI_SendItemQueryForList then
        local allItemEntries = {}
        
        -- Collect entries from inventory
        for _, item in ipairs(inventoryData) do
            if item.itemEntry then
                table.insert(allItemEntries, item.itemEntry)
            end
        end
        
        -- Collect entries from equipment
        ...
        -- Collect entries from bank
        for _, item in ipairs(bankData) do
            if item.itemEntry then
                table.insert(allItemEntries, item.itemEntry)
            end
        end
        
        -- Send packets for all unique items
        if #allItemEntries > 0 then
            _G.GameMasterUI_SendItemQueryForList(player, allItemEntries)
        end
    end
    --]]
...
function PlayerInventoryDataHandlers.getInventoryData(characterGuid, bagMapping)
    ...
    -- First pass: collect all inventory data and unique item entries
    ...
    repeat
        ...
        if not foundBagIds[bagId] then
            ...
            if bagId > 0 and bagId ~= 255 then
                if bagMapping[bagId] then
                    ...
                    -- Bag ID mapped to bag item
                else
                    -- Unknown bag ID found
                end
            end
        end
        foundBagIds[bagId] = foundBagIds[bagId] + 1

        -- Store raw inventory item data
        ...
        -- Collect unique item entries for batch query
        if itemEntry > 0 then
            uniqueItemEntries[itemEntry] = true
        end
    until not inventoryResult:NextRow()

    -- Second pass: batch query for all item template data
    ...
    for _, item in ipairs(rawInventoryData) do
        ...
        if enchantId and enchantId > 0 then
            -- Found enchanted inventory item (debug disabled)
        end
        ...
        if bagId == 0 then
            ...
            if slotId < 23 or slotId > 38 then
                includeItem = false
                -- Skipping non-backpack slot
            else
                -- Including backpack slot
            end
        end
...
function PlayerInventoryDataHandlers.getBankData(characterGuid)
    ...
    -- First pass: collect all bank data and unique item entries
    local rawBankData = {}
    local uniqueItemEntries = {}
    ...
    -- Third pass: process bank data with batched item template info
...
function PlayerInventoryDataHandlers.getEquipmentData(targetName, characterGuid)
    ...
    if targetPlayer then
        ...
        -- Player is online, using direct API
        ...
        for slot = 0, 18 do
            ...
            if item then
                ...
                -- Equipment slot processed
            end
        end
    else
        -- Player is offline, query database
        -- Player is offline, using database query
        ...
        for bankSlot = 67, 73 do
            ...
            if bankBag then
                ...
                if bagInfoQuery then
                    ...
                    -- Online player bank bag processed
                end
            end
        end
    else
        -- Player is offline, get bag sizes from database
        -- Player offline, checking database for bag sizes
...
function PlayerInventoryDataHandlers.createBagConfiguration(bagSizes, bagIdToInfo)
    ...
    -- Bag configuration created (debug output removed)

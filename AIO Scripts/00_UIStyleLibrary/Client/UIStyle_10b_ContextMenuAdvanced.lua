            ...
            if disabled then
                ...
                if hasArrow and menuList then
                    ...
                    menuItem:SetScript("OnEnter", function(self)
                        ...
                        -- Show rich tooltip if available
                        if self.tooltipTitle or self.tooltipText then
                            -- Create rich tooltip if not exists (with max width for square shape)
                            if not self.richTooltip then
                                self.richTooltip = CreateStyledTooltipFrame(self, "TOPRIGHT", 5, 0, 280)
                            end
                            
                            -- Build tooltip content
                            local tooltipContent = {}
                            if self.tooltipTitle then
                                table.insert(tooltipContent, { 
                            ...
                            if self.tooltipText then
                                -- Handle text that might have \n as literal characters
                                local processedText = self.tooltipText:gsub("\\n", "\n")
                            ...
                            if #tooltipContent > 0 then
                                self.richTooltip:SetContent(tooltipContent)
                                self.richTooltip:Show()
                            end
                        end
